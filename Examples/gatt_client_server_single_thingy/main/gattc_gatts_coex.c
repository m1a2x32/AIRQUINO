...
static const char remote_device_name[] = "Thingy3";
...
static void gattc_profile_event_handler(esp_gattc_cb_event_t event, esp_gatt_if_t gattc_if, esp_ble_gattc_cb_param_t *param)
{
    ...
    case ESP_GATTC_NOTIFY_EVT:
        ESP_LOGI(COEX_TAG, "ESP_GATTC_NOTIFY_EVT, receive notify value: %d", *(p_data->notify.value));
        memcpy(&airQual, &p_data->notify.value, p_data->notify.value_len);
        break;
...
static void gatts_profile_a_event_handler(esp_gatts_cb_event_t event, esp_gatt_if_t gatts_if, esp_ble_gatts_cb_param_t *param) {
    ...
    case ESP_GATTS_READ_EVT: {
        ...
        rsp.attr_value.len = sizeof(airQual); 
        memcpy(&rsp.attr_value.value, &airQual, sizeof(airQual));
    ...
    case ESP_GATTS_CONNECT_EVT: {
        ESP_LOGI(COEX_TAG, "%02x:%02x:%02x:%02x:%02x:%02x Connected\n",
