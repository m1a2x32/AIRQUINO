...
static const char remote_device_name[3][20] = {"Thingy", "Thingy3", "ESP_GATTS_DEMO_c"};
...
static void gattc_profile_a_event_handler(esp_gattc_cb_event_t event, esp_gatt_if_t gattc_if, esp_ble_gattc_cb_param_t *param)
{
    ...
    case ESP_GATTC_SEARCH_RES_EVT: {        
        ...
        && p_data->search_res.srvc_id.uuid.uuid.uuid128[5] == REMOTE_SERVICE_UUID[5]) {
            ...
            ESP_LOGI(GATTC_TAG, "Service A found");
            get_service_a = true;
    ...
    case ESP_GATTC_REG_FOR_NOTIFY_EVT: {
        ...
        if (count > 0){
            ...
            if (!descr_elem_result_a){
                ...
                ESP_LOGI(GATTC_TAG, "-%d-", descr_elem_result_a[0].uuid.len);
    ...
    case ESP_GATTC_NOTIFY_EVT:
        ESP_LOGI(GATTC_TAG, "Device A Received notify value:");
...
static void gattc_profile_b_event_handler(esp_gattc_cb_event_t event, esp_gatt_if_t gattc_if, esp_ble_gattc_cb_param_t *param)
{
    ...
    case ESP_GATTC_SEARCH_RES_EVT: {
        ...
        && p_data->search_res.srvc_id.uuid.uuid.uuid128[5] == REMOTE_SERVICE_UUID[5]) {
            ...
            ESP_LOGI(GATTC_TAG, "B: UUID128: %x:%x", p_data->search_res.srvc_id.uuid.uuid.uuid128[13], p_data->search_res.srvc_id.uuid.uuid.uuid128[12]);
            get_service_b = true;
    ...
    case ESP_GATTC_NOTIFY_EVT:
        ESP_LOGI(GATTC_TAG, "Device B Received notify value:");
        esp_log_buffer_hex(GATTC_TAG, p_data->notify.value, p_data->notify.value_len);
        break;
    case ESP_GATTC_WRITE_DESCR_EVT:
        if (p_data->write.status != ESP_GATT_OK){
...
static void gattc_profile_c_event_handler(esp_gattc_cb_event_t event, esp_gatt_if_t gattc_if, esp_ble_gattc_cb_param_t *param)
{
    ...
    case ESP_GATTC_SEARCH_RES_EVT: {
        ...
        if (p_data->search_res.srvc_id.uuid.len == ESP_UUID_LEN_128 
        && p_data->search_res.srvc_id.uuid.uuid.uuid128[0] == REMOTE_SERVICE_UUID[0]) {
            ESP_LOGI(GATTC_TAG, "C: UUID16: %x:%x", p_data->search_res.srvc_id.uuid.uuid.uuid128[13], p_data->search_res.srvc_id.uuid.uuid.uuid128[12]);
            get_service_c = true;
    ...
    case ESP_GATTC_NOTIFY_EVT:
        ESP_LOGI(GATTC_TAG, "Device C Received notify value:");
        esp_log_buffer_hex(GATTC_TAG, p_data->notify.value, p_data->notify.value_len);
        break;
    case ESP_GATTC_WRITE_DESCR_EVT:
        if (p_data->write.status != ESP_GATT_OK){
